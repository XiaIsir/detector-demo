...
class ELA(nn.Module):
    ...
    def __init__(self, channels: int, kernel_size: int = 7):
        ...
        while channels % ng != 0:
            ng -= 1
        self.gn = nn.GroupNorm(num_groups=ng, num_channels=channels)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.size()
        x_h = torch.mean(x, dim=3)  # (B, C, H)  - horizontal strip pooling
        x_w = torch.mean(x, dim=2)  # (B, C, W)  - vertical strip pooling
...
class PConv(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x1 = self.conv(x1)
        # When stride > 1, downsample untouched channels to match spatial size
        if self.stride != 1 and x1.shape[2:] != x2.shape[2:]:
            x2 = F.avg_pool2d(x2, self.stride, self.stride)
...
class AgentBottleneck(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, shortcut: bool = True, g: int = 1, e: float = 0.5, num_agent_tokens: int = 16):
        ...
        self.agent_attn = AgentAttention(c_, num_agent_tokens)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = self.cv1(x)
        out = self.agent_attn(out)
        out = self.cv2(out)
...
class FEM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gamma = w[:, 2, :].view(-1, w.shape[2], 1, 1)

        return alpha * f_up + beta * f_mid + gamma * f_low
...
class BiFPN_Concat(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        # Resize all feature maps to the same spatial size (use the first as anchor)
        target_h, target_w = x[0].shape[2], x[0].shape[3]
        resized = []
        for xi in x:
            if xi.shape[2] != target_h or xi.shape[3] != target_w:
                xi = F.interpolate(xi, size=(target_h, target_w), mode="nearest")
            resized.append(xi)
...
class EFBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.ela(out)
        out = self.cv3(out)
        return self.shortcut_proj(x) + out if self.add else out
