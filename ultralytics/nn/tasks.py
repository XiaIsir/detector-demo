...
# Register L-FAF custom modules for safe deserialization (PyTorch 2.4+).
# Without this, torch.load with weights_only=True replaces EFBlock/PConv/etc. with
# inert placeholders, silently destroying the model architecture.
try:
    torch.serialization.add_safe_globals([EFBlock, PConv, BiFPN_Concat, C2PSA, ELA])
except AttributeError:
    pass  # PyTorch < 2.4 — weights_only defaults to False, no issue
...
class SafeUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module, name):
        ...
        safe_modules = (
            ...
            "math",
            "numpy",
            "ultralytics.nn.modules.block",  # L-FAF custom modules: EFBlock, PConv, BiFPN_Concat, C2PSA, ELA
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            # PConv(dim, o_dim, n_div=4, kernel_size=3, stride=1, padding=1)
            # YAML args: [o_dim, kernel_size, stride] or [o_dim]
            k = args[1] if len(args) > 1 else 3
            s = args[2] if len(args) > 2 else 1
            args = [c1, c2, 4, k, s]  # n_div=4 (default)
        elif m is ELA:
            c1 = ch[f]
            args = [c1, *args]
            c2 = c1  # ELA preserves channel count
